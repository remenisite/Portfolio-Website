...
const Services = () => {
  return (
    <section id="service" data-aos="fade-up" className=" ">
      <div className="container text-center">
        <h2 className="text-[24px] font-normal font-main text-white">
          I like to make things easy and fun
        </h2>
        <h3 className=" px-[30px] text-center text-[40px] font-medium font-main mt-[40px] text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-[70px]">
          {services.map((service, index) => (
            <div
              key={index}
              className="group bg-[#1E1E1E] py-[35px] rounded-[10px] text-center flex justify-center flex-col items-center transform transition-transform duration-300 ease-in-out hover:scale-105 hover:-translate-y-3 hover:shadow-lg border-b-4 border-transparent hover:border-text transition-colors"
            >
              <div className="text-text text-[50px]">{service.icon}</div>
              <h4 className="text-[20px] font-black font-primary text-white pt-[42px] pb-[37px]">
                {service.title}
              </h4>
              <p className="text-[18px] font-normal font-second text-white w-[320px] text-center">
                {service.description}
              </p>
            </div>
          ))}
